...
void init()
{
//	SVSCTL = VLD3 | VLD1 | PORON; //  0xa8;	 Burnout detection set to 3.05v
	SVSCTL = VLD3 | PORON; //  0xa8;	 Burnout detection set to 2.8 v
        disableWatchDog();
	initClocks();
//	UART_Config();
        enableWatchDogIntervalMode();
	/* Clear P2.6 and P2.7 bits to default to GPIO */
#ifdef P2SEL2_    // Nonsense...
	P2SEL &= ~(BIT6|BIT7);
#endif
	P2DIR |=0x01;
        __eint();
...
#define WDT_TICKS_PER_SECOND  (1000*WDT_TICKS_PER_MILISECOND)
void enableWatchDogIntervalMode(void)
{
...
volatile uint32_t wdtCounter = 0, c1000=0;
...
uint32_t blink, ival=0;
void led_fblink ( int ms )
{ 
	ival = (ms * WDT_TICKS_PER_MILISECOND);
	if (ival) {
		blink = wdtCounter + ival; 
	} else {
		LED_OFF;
	}
}
...
//	if ((P5IN & QSPI_RST_N)==0) act_mask |= QSPI_RST_N;
	if ( c1000 == WDT_TICKS_PER_SECOND ){
		c1000=0;
//		TOGGLE_LED;	
		Second++;
		act_mask |= UPD_ADC;
