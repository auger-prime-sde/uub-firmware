...
//DN MISC_Config();
//DN IO_Config();
//DN led_fblink (50);           //set LED1 blink 5Hz
 UART_Config();
...
//
	UART_sprint ("Software Version: "); UART_sprint(VERSION);
 	UART_sprint(" \r");
 	UART_sprint("Serial: ");
...
//DN	P5DIR |= B3V3_QSPI_B | NMI_FPGA | RST_FPGA; // 
//DN	P1OUT =(FPGA_CORE_PS);	// Enable Powersupplies
	delay (200); // wait 200 ms
   //DN                     P1OUT |= (PS_AN_EN | PS_EXT1_24V_EN | PS_EXT2_24V_EN);  // Enable ADC Powersupplies 
      //DN                  P2OUT |= (PS_PMT_12V_EN | PS_RADIO_12V_EN);     // Enable ADC Powersupplies
//DN	POR_FPGA ();	// Power on reset of FPGA
	UART_sprint(" ........ ");
...
//DN led_fblink (0);           //set LED1 blink 5Hz
}
void check_stat()
{
                if (P2IN & DONE) {              // FPGA booted
...
//DNif (act_mask & UPD_ADC ) {
//DN                        act_mask &= ~UPD_ADC;
//DN                        adcctl = ADC_DOCONV;
//DN                        adc_start_conversion();
//DN        }
...
void Parse_UART()
{
	...
	switch (uart_buffer[0])
	{
		...
		case 'l':
			...
			break;
		case 'p':
//			P1OUT &= ~(ADC_CORE_PS);	// Disable ADC Powersupplies
			P1OUT &= ~(PS_AN_EN | PS_EXT1_24V_EN | PS_EXT2_24V_EN);	// Enable ADC Powersupplies
			P2OUT &= ~(PS_PMT_12V_EN | PS_RADIO_12V_EN);	// Disable ADC Powersupplies
			break;
		case 'P':
//			P1OUT |= (ADC_CORE_PS);	// Enable ADC Powersupplies
			P1OUT |= (PS_AN_EN | PS_EXT1_24V_EN | PS_EXT2_24V_EN);	// Enable ADC Powersupplies
			P2OUT |= (PS_PMT_12V_EN | PS_RADIO_12V_EN);	// Enable ADC Powersupplies
			break;
		default:
