	...
	if (argc == 6) { //option by arguments
		...
		if (argv[1][0] == '-' && argv[1][1] == 'h') {	//-h or -help
			printf(
					"#################################################################\n");
			printf(
					"# Preliminary tool to verify if ADC of UUB are stuck or not     #\n");
			printf(
					"# HELP: adc-check2 min_base max_base min_stdev max_stdev reset  #\n");
			printf(
					"# min_base: baseline low limit / max_base: baseline high limit  #\n");
			printf(
					"# min_stdev: stdev low limit / max_stdev:stdev high limit       #\n");
			printf(
					"# baseline limits to know when adc stucks outside normal range  #\n");
			printf(
					"# min_stdev to detect ADC stuck in one unique value             #\n");
			printf(
					"# max_stdev to detect ADC toggling between two values           #\n");
			printf(
					"# Reset enables with one the reset if ADC are stuck, 0=noreset  #\n");
			printf(
					"# If without arguments, default values are 250 350 0.01 2.5 0   #\n");
			printf(
					"# A.Travaini (Based on R.Sato soft)- 2019 - andres@auger.org.ar #\n");
			printf(
					"#################################################################\n");
			exit(0);
		} else {
				...
				if (saturated == 0) {//if not saturated event(put to all channels later)
					for (i = 0; i < N_MAX_SAMP; i++) { //Just take trace before trigger(<667) if triggering with signal
						...
						for (ch = 0; ch < 5; ch++) {	//sweep all channels
							...
							adc_values[ch * 2][i + l * N_MAX_SAMP] =
									(float) (ev->fadc_raw[ch][offset] & 0xFFF);
							//HG channel, 12 bits of ADC, bit 16 to 27.
							adc_values[ch * 2 + 1][i + l * N_MAX_SAMP] =
									(float) ((ev->fadc_raw[ch][offset] >> 16)
											& 0xFFF);
						}
					}
				} else {
...
int ADC_status(float * a) {
	...
	for (i = 0; i < 10; i++) {	//std_dev
		...
		if (std_dev[i] <= min_stdev || std_dev[i] >= max_stdev
				|| mean[i] >= max_base || mean[i] <= min_base) {
